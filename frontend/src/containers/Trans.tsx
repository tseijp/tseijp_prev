...
export const TransArea :FC<BindsProps> = ({spring, bind, size=1}) => {
    const background = spring.scale.to((s:number) => `linear-gradient(90deg,rgba(0,0,0,0),rgba(0,0,0,${s-1}))`)
    const width    = spring.rotateZ.to((r:number) => `${ 50*size*( Math.cos(r/90*Math.PI)+1.5) }px`)//s*1.5~2.5
...
export const TransToggle : FC<BindsProps> = ({spring, bind, size=1}) => {
    const styles = useMemo<CSSProperties[]>(()=> [
        {position:"absolute",top:50*size,right:50*size,fontSize:50*size},
...
export const TransContainer : FC<BindsProps> = ({children, bind, spring, size=1}) => {
    const margin = `calc(${50*size*2}px - 2%) 0px 0px 0px`  //`calc(${size}*2 - 4%) 0px 0px 0px`
    const style = { position:"fixed",height:`96%`,top:"2%",right:0,zIndex:1,overflow:"hidden", }
    const width = spring.rotateZ.to((r:number)=> `${ 50*size * (Math.cos(r/90*Math.PI)+1) }px` )
    return (
...
export const TransItem :FC<BindsProps> = ({children, size=1, /*spring, width*/}) => { // TODO1701
    const styles = useMemo<CSSProperties[]>( () => [
